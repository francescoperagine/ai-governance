...
_Project-specific architectural rules that must never be broken. Examples:_

- **Architecture over speed.** Don't bypass data layers to save effort.
- **State-driven UI.** Views derive from state, never imperative DOM manipulation.
- **Data separation.** Rules and constants live in the data layer, never in components.
- _...add yours._

## Handoff Mode Discipline

Handoff files (`PROMPT.md` / `HANDOFF.md`) declare `mode:` in front-matter and repeat it as a
...
Before executing any handoff, verify its paths and `inc_base` against the real repo. On any
discrepancy: **stop and report before proceeding.**

## Verify Before Writing Code

- Check proposed changes against existing types, interfaces, and computed accessors *before*
  emitting code.
- If the type-check fails, refactor to match the domain types — do not widen types, cast, or
  suppress the error to make the gate green.
- If reality contradicts the spec, the spec is wrong: stop and report, don't improvise.

## Feature Lifecycle

Feature-tier work lives in `features/<name>/` (kebab-case): `SPEC.md` (live ledger) plus one
entry-point handoff. Planning session writes them; a fresh session executes them.

**Closing a feature is part of the last increment:** move `SPEC.md` to
`docs/archive/features/INC-<closing>_<name>.md`, delete `features/<name>/`, update
`docs/PROJECT.md` (and `docs/ROADMAP.md` if a milestone closed). A folder left in `features/`
falsely claims work is in progress.

## Task Completion (MANDATORY)

A task is NOT complete unless:

- Live code implemented and the gate is green — `[build command]` **and** `[test command]`.
  Run the gate **only when live code was touched**; doc- and spec-only turns skip it.
- Gate result recorded as command + exit code, not as a claim.
- `CHANGELOG.md` updated with a unique `INC-XXX` read from the CHANGELOG head.
- ADR created **if a decision with real alternatives was made**; otherwise
  `ADR: none (mechanical)`.
- `features/<name>/SPEC.md` updated; `docs/ARCHITECTURE.md` updated if topology changed.
- If this closed the feature: retirement steps above performed.
- Localization always in **[your language]**.

**Where knowledge goes.** Long-term direction → `docs/ROADMAP.md`; current focus →
`docs/PROJECT.md`; technical decision → `docs/adr/`; in-progress work →
`features/<name>/SPEC.md`; completed change → `CHANGELOG.md`; current system shape →
`docs/ARCHITECTURE.md`; retired SPECs → `docs/archive/features/`.
`docs/todo.md` is the maintainer's personal backlog — **do not touch unless explicitly asked.**

**Self-conclude.** Gate green → checklist done → write the next handoff if work remains →
short report → **end. Do not ask for further instructions.**

## Coding Standards & CLI

- **[Standard 1]**
- **[Standard 2]**
- **CLI:** `dev` · `build` (type-check + build, **mandatory gate**) · `test` · `lint` ·
  `format` · `docs:check` (see governance template §9).

## Scope Discipline

Modify only files explicitly in scope. When delegating, instruct workers to stay strictly in
scope. Never let a formatter reformat files unrelated to the task. Verify the diff before
consolidating.
```
